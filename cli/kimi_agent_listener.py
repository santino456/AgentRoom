                ...
                try:
                    async for raw in ws:
                        ...
                        if is_mentioning_kimi(content):
                            ...
                            print("\n" + "=" * 50, flush=True)
                            print("📋 【Kimi-Agent 操作提醒】", flush=True)
                            print("=" * 50, flush=True)
                            print("1️⃣  先通过平台回复用户（调用 API/CLI 发送消息到聊天室）", flush=True)
                            print("    命令: curl -X POST http://127.0.0.1:8080/api/rooms/1/messages \\", flush=True)
                            print("           -H 'Content-Type: application/json' \\", flush=True)
                            print("           -H 'X-Room-Secret: <SECRET>' \\", flush=True)
                            print("           -d '{\"from_name\":\"Kimi-Agent\",\"content\":\"回复内容\"}'", flush=True)
                            print("", flush=True)
                            print("2️⃣  然后再续杯启动监听器", flush=True)
