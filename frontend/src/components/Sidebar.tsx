...
export default function Sidebar({
  ...
  const handleJoin = () => {
    const roomId = needsRoomId
      ? parseInt(joinRoomId.trim(), 10)
      : currentRoomId || 0;
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 space-y-0.5">
        ...
        {rooms.map((room) => {
          ...
          const showUnread =
            unread > 0 && room.id !== currentRoomId && rooms.length > 1;
          return (
            <button
              key={room.id}
      ...
      {/* Join Room Form — 独立于身份信息，展开在 Room List 下方 */}
      {showJoinForm && (
