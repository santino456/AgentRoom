...
export async function getMessages(
  ...
  const r = await fetch(`${API_BASE}/rooms/${roomId}/messages?limit=${limit}`, {
    signal,
    credentials: "include",
  });
  return r.json();
...
export async function getMembers(roomId: number, signal?: AbortSignal) {
  const r = await fetch(`${API_BASE}/rooms/${roomId}/members`, {
    signal,
    credentials: "include",
  });
  return r.json();
...
export async function getAgentStatus(roomId: number, signal?: AbortSignal) {
  const r = await fetch(`${API_BASE}/rooms/${roomId}/agent-status`, {
    signal,
    credentials: "include",
  });
  return r.json();
...
export async function updateMemberRole(
  ...
  const r = await fetch(
    `${API_BASE}/rooms/${roomId}/members/${memberId}/role`,
    {
      method: "PUT",
      headers,
