...
export default function App() {
  ...
  const loadUnreadCount = useCallback(
    async (roomId: number) => {
      try {
        ...
        const res = await fetch(
          `${API_BASE}/rooms/${roomId}/messages/unread-count`,
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const res = await fetch(
          `${API_BASE}/rooms/${currentRoomId}/attachments`,
          {
            method: "POST",
            headers,
            body: formData,
  ...
  const joinRoom = async (roomId: number, name: string, secret: string) => {
    ...
    if (room?.secret || secret)
      headers["X-Room-Secret"] = secret || room?.secret || "";
    ...
    try {
      ...
      const memRes = await fetch(`${API_BASE}/rooms/${roomId}/members`, {
        credentials: "include",
        headers: memHeaders,
      });
      const mems = await memRes.json();
  ...
  const saveEdit = async (msgId: number, content: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/rooms/${currentRoomId}/messages/${msgId}`,
        {
          method: "PUT",
          headers,
  ...
  const handleDeleteRoom = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to disband this room? All messages and members will be deleted. This cannot be undone.",
      )
    )
      return;
    try {
      ...
      >
          ...
          {showAgentsPage ? (
            ...
            <>
              ...
              <div className="px-5 pt-3 relative flex items-center justify-end">
                ...
                {showSearch && (
                  <div
                    className="absolute left-5 bottom-0 translate-y-full mt-1 text-[10px] px-1 z-10"
                    style={{ color: "var(--text-muted)" }}
                  >
                    {isSearching
                      ? "Searching..."
        ...
        <Suspense fallback={null}>
          <MemberList
            ...
            myName={memberName}
            onRefreshMembers={() =>
              currentRoomId && loadRoomData(currentRoomId)
            }
